...
document.addEventListener("DOMContentLoaded", (event) => {
    ...
    function addScrollEvents() {
        ...
        // Timelines
        const packingTimeline = gsap.timeline({paused: true});
        packingTimeline.fromTo(rectData, {
            scale: 0,
            opacity: 0,
            width: 0,
            height: 0,
            rx: (index) => packedRectData[index].width / 2,
            ry: (index) => packedRectData[index].height / 2,
        }, {
        ...
        const categoriesDistributionTimeline = gsap.timeline({paused: true});
        ...
        const dataSharedTimeline = gsap.timeline({paused: true});
        ...
        // Bar chart
        // actorsColumnTimeline.to(svgRects.nodes(), {
        //     rx: 0,
        //     ry: 0,
        //     x: "+=" + Math.random() * 200,
        //     duration: animationDuration,
        ...
        svgRects.nodes().forEach((rect, index) => {

            actorsColumnTimeline.to(rect, {
                rx: 0,
                ry: 0,
