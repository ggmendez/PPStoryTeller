...
document.addEventListener("DOMContentLoaded", (event) => {
    ...
    let movedRectData, rectData, svgRects, svgCategoryGroups, categoryStartPositions, packedRectData;
    ...
    function addScrollEvents() {
        ...
        // .fromTo(svgCategoryGroups.nodes(), {
        //     x: (i) => categoryStartPositions[i].x,
        //     y: (i) => categoryStartPositions[i].y
        // }, {
        //     x: (d, i) => {
        //         const columns = Math.ceil(Math.sqrt(svgCategoryGroups.size()));
        //         const column = i % columns;
        //         const cellWidth = width / columns;
        //         return column * cellWidth + cellWidth / 2;
        //     },
        //     y: (d, i) => {
        //         const columns = Math.ceil(Math.sqrt(svgCategoryGroups.size()));
        //         const row = Math.floor(i / columns);
        //         const cellHeight = 50; // Adjust height as needed
        //         return height - (svgCategoryGroups.size() / columns * cellHeight) + row * cellHeight + cellHeight / 2;
        //     },
        //     duration: animationDuration,
        //     ease: "power1.inOut",
        //     stagger: { amount: animationDuration / 2 },
        ...
        //             const cellHeight = 50; // Adjust height as needed
        //             const x = column * cellWidth + cellWidth / 2;
        //             const y = height - (svgCategoryGroups.size() / columns * cellHeight) + row * cellHeight + cellHeight / 2;
        ...
        actorGroups.forEach((actorGroup, index) => {
            ...
            const labelWidthScaled = label.node().getBBox().width * actorGroupScale;
        ...
        // Initial conditions
        // gsap.to(svgCategoryGroups.nodes(), { opacity: 0, duration: 0 });
        ...
        function getPackedDataForSplitRects(point1, point2, totalHeight, targetSize) {
            ...
            function calculateColumnMajorPositions2(rectData, startX, width, height, rightToLeft = false) {
                ...
                const rectDiameter = 2 * targetSize;
                ...
                rectData.forEach((d, i) => {
                    ...
                    if (rightToLeft) {
                        const initialX = startX + col * (rectDiameter + padding) + padding;
                        const rightmostX = startX + (columns - 1) * (rectDiameter + padding) + padding;
        ...
        function onExplanationEnter(element, index) {
            ...
            console.log("mainTimeline.previousLabel()");
            console.log(mainTimeline.previousLabel());

            console.log("mainTimeline.nextLabel()");
            console.log(mainTimeline.nextLabel());
            ...
            if (element.id === "divPiecesOfData") {
                ...
                mainTimeline.tweenFromTo("dataShared", "actors");
            } else if (element.id === "divTotalActorCategories") {
                mainTimeline.tweenFromTo("actors", "actorsColumn");
        ...
        function onExplanationLeave(element, index) {
            if (element.id === "divPiecesOfData") {
                ...
                mainTimeline.tweenFromTo("actors", "dataShared");
            } else if (element.id === "divTotalActorCategories") {
                mainTimeline.tweenFromTo("actorsColumn", "actors");
            } else if (element.id === "divDataPerActor") {
